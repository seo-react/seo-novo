...
const CATEGORY_MAP = {
  ...
  'Management': 7,
  'Optimization': 8,
  'Digital Marketing': 9, // Evite duplicar o ID 7
};
...
const BlogGrid = () => {
  ...
  // Gerar ID seguro para tab
  const getTabId = (category) => category.toLowerCase().replace(/\s+/g, '-');
  ...
  return (
    <div className="blog-one pb-140">
      <div className="tabs-one">
                  ...
                  {Object.keys(CATEGORY_MAP).map((category) => (
                    <li className="nav-item" role="presentation" key={category}>
                      <button
                        ...
                        type="button"
                        role="tab"
                        aria-controls={`pills-${getTabId(category)}`}
                        aria-selected={activeTab === category}
                      >
                        <img src={`/assets/img/icon/${getIconName(category)}.svg`} className="no-active" alt={category} />
                        <img src={`/assets/img/icon/${getIconName(category)}-white.svg`} className="active-tab-icon" alt={category} />
                        <span>{category}</span>
                      </button>
                    </li>
                  ))}
        ...
        {/* Conteúdo dos Posts (com tab-content) */}
        <div className="row">
          <div className="col-xxl-12">
            <div className="tab-content" id="pills-tabContent">
              <div
                className="tab-pane show active fadeInUp"
                id={`pills-${getTabId(activeTab)}`}
                role="tabpanel"
                aria-labelledby={`pills-${getTabId(activeTab)}-tab`}
                tabIndex="0"
              >
                <div className="container">
                  <div className="row">
